...
```{r Import}
df<-vroom::vroom("../data/data_file.tsv.xz") %>% janitor::clean_names()
...
Choose a sensor to plot. The names are based on the ward and room. For example. J06_A13_03 is ward J6, room 3.

```{r}
selectInput("sensor", label = "Sensor to plot:",
            choices = unique(df$id), selected = "1152")
```

Sensors measure every 10s. Down-select the plotting frequency to visualise the data more clearly. This only affects the time-series currently.
```{r}
sliderInput("frequency", label = "Moving averages smoothing every:",
...
```{r}
renderValueBox({
  m <- median(a()$co2_ppm,na.rm = TRUE)#formatC(downloadRate(), digits = 1, format = "f")
  valueBox(
    value = m,
    icon = "fa-area-chart",
...
```{r}
renderValueBox({
  m <- median(a()$temperature_c,na.rm = TRUE)#formatC(downloadRate(), digits = 1, format = "f")
  valueBox(
    value = m,
    icon = "fa-area-chart",
...
### **Absolute Humidity** Violin plots
